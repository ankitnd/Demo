...
?>
    ...
    public function fetch_permission_alll() {
        $l_id = $this->input->post('logtype');
        $ldata = $this->cm->select_data("logtype", "logtype_id", $l_id);
...
?>
				 <label for="pwd">Permission :-</label>
	                      ...
	                      <?php foreach ($f_module as $key => $value) { ?>
                            <h3><?php echo $value->f_module_name; ?><input type="checkbox" name="fp[]" value="<?php echo $value->f_module_name; ?>" <?php if (isset($ldata->f_permission) && in_array($value->f_module_name, explode(",", $ldata->f_permission))) {
                echo "checked";
            } ?>></h3>
                            <?php foreach ($m_module as $m) {
                if ($m->f_module_id == $value->f_module_id) {
?>
                            <label for="pwd"><?php echo $m->module_name; ?> <input type="checkbox" name="m_all[]" value="<?php echo $m->module_name; ?>"<?php if (isset($ldata->m_permission) && in_array($m->module_name, explode(",", $ldata->m_permission))) {
                        echo "checked";
                    } ?>>:</label>
                            <br>
...
?> 
                        <label style="width:30%;font-weight: normal;"><?php echo $l->name; ?> </label> 
                                    <label class="radio-inline">
                                      <input type="checkbox" name="f_all[]"  value="<?php echo $l->name; ?>" <?php if (isset($ldata->permission) && in_array($l->name, explode(",", $ldata->permission))) {
                                echo "checked";
                            } ?>>Yes
                                    </label>
...
?>
    ...
    public function change_f_mod() {
        ...
        foreach ($data as $key => $value) { ?>
			<label for="pwd"><?php echo $value->module_name; ?> <input type="checkbox" name="m_all[]" value="<?php echo $value->module_name; ?>" onclick="l_chnage_mod(<?php echo $value->m_module_id; ?>)">:</label>
			<br>
			<div id="l_change_mod<?php echo $value->m_module_id; ?>"></div>
		<?php
        } ?>

		<script type="text/javascript">
  function l_chnage_mod(id)
  {
   
      var a = id;
      $.ajax({
        url:"<?php echo base_url(); ?>settings/change_l_mod",
        type:'POST',
        data:{a_name:a},
        success:function(res){
          $('#l_change_mod'+id).html(res);
        }
      });
  }
</script>
    ...
    public function change_l_mod() {
        $id = $this->input->post('a_name');
        $select_logtype = $this->cm->select_data("logtype", "logtype_name", 'Admin');
        $data = $this->cm->filter_data("l_module", "m_module_id", $id);
        foreach ($data as $k => $l) { ?>
		   <label style="width:30%;font-weight: normal;"><?php echo $l->name; ?> </label> 
                                    <label class="radio-inline">
                                     <input type="checkbox" name="f_all[]"  value="<?php echo $l->name; ?>" <?php if (isset($select_logtype->permission) && in_array($l->name, explode(",", $select_logtype->permission))) {
                echo "checked";
            } ?>>Yes
                                    </label>
                                    <br>
